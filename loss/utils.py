...
def recompute_memory(epoch, net, lemniscate, trainloader):
    ...
    net.eval()
    trainFeatures = lemniscate.memory.t()
    # trainTargets = lemniscate.memory_targets.t()
    batch_size = 100

    with torch.no_grad():
        transform_bak = trainloader.dataset.transform
        temploader = torch.utils.data.DataLoader(trainloader.dataset, batch_size=batch_size, shuffle=False, num_workers=4)
        for batch_idx, (inputs, _, _, targets, domainIDs, indexes) in enumerate(temploader):
            ...
            # trainTargets[:, batch_idx * batch_size:batch_idx * batch_size + c_batch_size] = targets.data.t()
    ...
    lemniscate.memory = trainFeatures.t()
    # lemniscate.memory_targets = trainTargets.t()
    # lemniscate.memory_targets = trainLabels
...
class CEandMAE(torch.nn.Module):
    ...
    def forward(self, pred, labels_one_hot):
        return self.alpha * self.ce(pred, labels_one_hot) + self.beta * self.mae(pred, labels_one_hot)
...
def noisify_sketchy25_asymmetric(y_train, noise, random_state):
    ...
    if n > 0.0:
        ...
        P[4, 4], P[4, 3] = 1.-n, n
        P[1, 4], P[1, 1] = 1.-n, n
        P[10, 16], P[10, 10] = 1.-n, n
        P[15, 23], P[15, 15] = 1.-n, n
        P[23, 15], P[23, 23] = 1.-n, n
        P[21, 17], P[21, 21] = 1.-n ,n
        P[14, 0], P[14, 14] = 1.-n, n
        P[24, 2], P[24, 24] = 1.-n, n
        P[18, 19], P[18, 18] = 1.-n, n
        P[6, 12], P[6, 6] = 1.-n, n
        P[11, 9], P[11, 11]= 1.-n, n
        P[20, 13], P[20 ,20]= 1.-n, n
        ##
...
# get asymmtric noisy label 一个类别变为其他类别的可能性不同
def noisify_multiclass_asymmetric(gt_label, ratio, class_num):
    count= 0
    noisy_label= np.copy(gt_label)
    p= class_num* 5
    noise_pakage= np.zeros((class_num, p))
    # 标签转化矩阵
    for i in range(class_num):
        for j in range(p):
            random_label= np.random.randint(0, class_num)
            while random_label== i:
                random_label= np.random.randint(0, class_num)
            noise_pakage[i, j]= random_label;
    # 产生标签噪声
    for i in range(gt_label.shape[0]):
        if(np.random.random()<=ratio):
            noisy_label[i]= random.choice(noise_pakage[gt_label[i]])
            count+=1
    print("错误标签：%d 正确标签：%d 错误率：%.2f" %(count, gt_label.shape[0], count/gt_label.shape[0]))
    return noisy_label

# get pair flip noisy label 一个类别只会变为另一个指定类别
...
# get symmetric noisy label 一个类别变为任何其他的类别的可能性是相同的
