...
def a_img_aug(resize_size= 256, crop_size= 224, domain=''):
    return transforms.Compose([transforms.ToPILImage(),
        transforms.Resize((resize_size, resize_size)),
        transforms.RandomResizedCrop(scale=[0.2, 1], size=crop_size),
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        torchvision.transforms.Normalize([0.5114531,  0.4790816,  0.44287592],
                                         [0.24088411, 0.23595929, 0.22870816])
    ])

def c_img_aug(resize_size= 256, crop_size= 224, domain=''):
    return transforms.Compose([transforms.ToPILImage(),
        transforms.Resize((resize_size, resize_size)),
        transforms.RandomResizedCrop(scale=[0.2, 1], size=crop_size),
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        torchvision.transforms.Normalize([0.5967357,  0.575849,   0.55142397],
                                         [0.2984169,  0.29634935, 0.30053937])
    ])

def p_img_aug(resize_size= 256, crop_size= 224, domain=''):
    return transforms.Compose([transforms.ToPILImage(),
        transforms.Resize((resize_size, resize_size)),
        transforms.RandomResizedCrop(scale=[0.2, 1], size=crop_size),
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        torchvision.transforms.Normalize([0.74720377, 0.73447365, 0.7262723 ],
                                         [0.27246457, 0.27875733, 0.28361407])
    ])

def r_img_aug(resize_size= 256, crop_size= 224, domain=''):
    return transforms.Compose([transforms.ToPILImage(),
        transforms.Resize((resize_size, resize_size)),
        transforms.RandomResizedCrop(scale=[0.2, 1], size=crop_size),
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        torchvision.transforms.Normalize([0.60925746, 0.5769886, 0.5440728 ],
                                         [0.24070711, 0.24153146, 0.24660982])
    ])
...
class home_dataset(Dataset):
    def __init__(self, args, sample_ratio= None, transform=None, mode=None, pred= [], probability= [], s_noise_file='', t_noise_file='', domain= '', pseudo_label= ''):
        ...
        if args.dataset == 'office_home':
            ...
            if ss == 'a':
                s = 'Art'
                self.s_aug_transform = a_img_aug()
            elif ss == 'c':
                s = 'Clipart'
                self.s_aug_transform = c_img_aug()
            elif ss == 'p':
                s = 'Product'
                self.s_aug_transform = p_img_aug()
            elif ss == 'r':
                s = 'Real_World'
                self.s_aug_transform = r_img_aug()
            else:
                raise NotImplementedError
            if tt == 'a':
                t = 'Art'
                self.t_aug_transform = a_img_aug()
            elif tt == 'c':
                t = 'Clipart'
                self.t_aug_transform = c_img_aug()
            elif tt == 'p':
                t = 'Product'
                self.t_aug_transform = p_img_aug()
            elif tt == 'r':
                t = 'Real_World'
                self.t_aug_transform = r_img_aug()
            else:
                raise NotImplementedError
        if domain== 's':
            self.aug_transform= self.s_aug_transform
        else:
            self.aug_transform= self.t_aug_transform

        s_tr_path = 'dataset/datasets/data_split/office-home/' + s + '_train.txt'
        s_ts_path = 'dataset/datasets/data_split/office-home/' + s + '_test.txt'
        t_tr_path = 'dataset/datasets/data_split/office-home/' + t + '_train.txt'
        t_ts_path = 'dataset/datasets/data_split/office-home/' + t + '_test.txt'
        ...
        #     self.eval_data, self.eval_label= make_dataset(train_path, None)
...
class home_loader():
    ...
    #whole data set
        ...
        #evaluating training data set
            ...
            test_tr_loader= DataLoader(dataset= test_tr_dataset, batch_size=self.batch_size*2, shuffle= False, num_workers=self.num_workers, drop_last= False)
