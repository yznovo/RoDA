...
def make_dataset(image_list, labels):
    if labels:
        ...
        if len(image_list[0].split()) > 20:
            images = [(val.split()[0],
                       np.array([int(la) for la in val.rsplit(' ', 1)]))
                      for val in image_list]
        else:
            images = [val.rsplit(' ', 1)[0] for val in image_list]
            labels = [int(val.rsplit(' ', 1)[1]) for val in image_list]
...
class home_dataset(Dataset):
    def __init__(self, args, sample_ratio= None, transform=None, mode=None, pred= [], probability= [], s_noise_file='', t_noise_file='', domain= '', pseudo_label= ''):
        ...
        s_tr_path = 'dataset/adaptiope/' + s + '_train.txt'
        s_ts_path = 'dataset/adaptiope/' + s + '_test.txt'
        t_tr_path = 'dataset/adaptiope/' + t + '_train.txt'
        t_ts_path = 'dataset/adaptiope/' + t + '_test.txt'
